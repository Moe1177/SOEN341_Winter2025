...
public class AuthController {
    ...
    /**
     * Handles the login request by authenticating the user's credentials and generating a JWT token if the authentication is successful.
     * The method checks if the provided username and password match the records in the database and whether the user's email is verified.
     * If authentication succeeds, a JWT token is generated and returned. If authentication fails or the email is not verified, an appropriate error response is returned.
     *
     * @param username The username of the user attempting to log in.
     * @param password The password provided by the user for authentication.
     * @return A {@link ResponseEntity} containing the JWT token on successful login, or an error message with the corresponding HTTP status.
     */
    @PostMapping("/login")
    ...
    /**
     * Registers a new user by validating the uniqueness of the provided username and email,
     * creating a new user record in the database, and sending a verification code to the user's email.
     * The method checks if the username or email already exists in the system and returns an appropriate
     * error response if they do. A random verification code is generated and sent via email,
     * with an expiration time of 10 minutes. The user is initially marked as unverified.
     *
     * @param username The desired username for the new user.
     * @param email The desired email for the new user.
     * @param password The password for the new user.
     * @return A {@link ResponseEntity} indicating the result of the registration process.
     *         If successful, a status of {@code CREATED} is returned with a message prompting the user to check their email.
     *         If the username or email already exists, a {@code CONFLICT} status is returned with an error message.
     */
    ...
    /**
     * Verifies the provided email verification code for a given user.
     * The method checks if the user exists, if the user is already verified, and whether the provided
     * verification code matches the one stored in the database. It also checks if the verification code
     * has expired. If valid, the user's email is marked as verified and the verification code is cleared.
     * If any condition fails, an appropriate error response is returned.
     *
     ...
     *         If the verification is successful, a {@code OK} status is returned with a success message.
     *         If the user is not found, the code is invalid, or it has expired, an error response with the appropriate status is returned.
     */
    ...
    /**
     * Resends the email verification code to the user if they are not already verified.
     * The method checks if the user exists, if the user is already verified, and then generates a new
     * verification code with a new expiration time. A new verification code is sent to the user's email.
     *
     * @param username The username of the user who is requesting to resend the verification code.
     * @return A {@link ResponseEntity} indicating the result of the resend request.
     *         If the user is not found, a {@code NOT FOUND} status is returned.
     *         If the user is already verified, a {@code BAD REQUEST} status is returned.
     *         If the resend is successful, an {@code OK} status is returned with a success message.
     */
